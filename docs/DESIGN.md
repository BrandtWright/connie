...
│  src/config/user.yml          User config template              │
...
│  /usr/local/lib/connie/config/   defaults.yml + project/user.yml│
...
Claude Code loads `CLAUDE.md` files from several scopes (project `./CLAUDE.md`,
`~/.claude/CLAUDE.md`, and others), documented in the [Claude Code memory
documentation][claude-code-memory]. **connie writes none of them.** Instead it
appends its own context to Claude's *system prompt* at launch, via the
`--append-system-prompt` flag, and leaves the project's `CLAUDE.md` (and
`CLAUDE.local.md`) to load natively and untouched.

This is a deliberate change from connie's earlier design, which baked a
managed-policy `/etc/claude-code/CLAUDE.md` into the image and assembled a
`~/.claude/CLAUDE.md` from host files. Appending to the system prompt is:

- **Non-invasive.** connie creates no file in the image, the state dir, or the
  project, so it cannot collide with, shadow, or override a project's own
  `CLAUDE.md`. A repo that checks critical guidance into `CLAUDE.md` keeps it
  intact.
- **Semantically right.** "You are in a hardened container, N CPUs, read-only
  ...
  where environment framing belongs; `CLAUDE.md` is for the project's own
  instructions.
- **Rebuild-free.** The payload is computed host-side and injected as a
  run-time Compose `command:`, so editing context never triggers an image
  rebuild (the old build-arg bake did).
- **Cached.** The appended text is part of the system-prompt prefix, so after
  the first turn it is served from Claude Code's prompt cache, not re-billed.

connie stays Claude-specific here: it can rely on `--append-system-prompt`
because the base image pins the Claude installer by SHA.

### The payload: four scopes

`_resolve_context` assembles one markdown payload host-side from up to four
scopes, in order:

| Scope | Source (host) | Heading | Included |
| --- | --- | --- | --- |
| Application | generated from the merged config | `# Connie Container Environment` | always |
| Machine | `${XDG_CONFIG_DIRS%%:*}/connie/context.md` | `## Machine Context` | if present |
| User | `$XDG_CONFIG_HOME/connie/context.md` | `## User Context` | if present |
| Project | `$CONFIG_DIR/projects/<slug>/context.md` | `## Project Context` | if present |

The **application** scope is generated by `_generate_connie_context` from the
merged config (the same merge described under Config Merge Flow above):
filesystem layout, available tooling, build-time commands, mounts, ports,
environment, resource limits, and security posture. It is always present.

The other three are plain `context.md` files beside each config layer's
`config.yml`, read verbatim and added under a `## … Context` heading only when
the file exists and is non-empty. Four independent files (rather than one
merged value) keep every scope present at once instead of a more-specific layer
overriding a broader one. The **project** scope here is connie's *out-of-repo*
per-project context — personal notes under connie's config dir that complement,
and never collide with, the repo's own `CLAUDE.md`.

### Injection

`_generate_override` injects the payload by emitting the Compose `command:` as
a JSON argv array when the launch command is `claude`:

```text
command: ["claude", "--append-system-prompt", "<payload>"]
```

An argv array (not an `sh -c` string) keeps the multi-line payload a single
verbatim element, so there is no shell quoting to get wrong: `yq -o=json -I=0`
emits it on one line with `\n` escapes that YAML decodes back to the original
multi-line value on parse, and the same `$`→`$$` doubling that protects every
other override value keeps any `$` in the payload literal. Any other
`start_cmd` is passed through as a scalar string and gets **no** injection —
connie will not append a flag a different tool may not understand; read
`connie context` and wire the output in yourself.

### Previewing

`connie context [dir]` prints the exact payload connie will append — the same
`_resolve_context` output `cmd_run` feeds to Claude, so the preview is provably
what Claude receives — without launching the container or writing anything. A
one-line diagnostic goes to stderr and the payload to stdout, so
`connie context > preview.md` captures just the payload. It is the cheapest way
to verify what Claude will receive before a run.

Because connie no longer mirrors Claude's own memory hierarchy, the preview
shows only connie's injected context. The project's `CLAUDE.md` /
`CLAUDE.local.md` and any `.claude/rules/*.md` load on Claude's side and are
intentionally out of scope for the preview.
...
| Integration | `tests/integration/` | In-process helpers that touch the environment but not Docker: `_migrate_project`, `_register_project`, `_find_project_root`, `cmd_init`, `_resolve_context` (context-payload assembly), `_prepare` and `_run_compose` (via shell-function stubs for docker), `_confirm`, and the `_require_yq` v4 sniff. |
| CLI | `tests/cli/` | Top-level subcommand behaviour observable via stdout/stderr/exit code: `connie help`, `connie config`, `connie edit`, `connie context`, `connie list`, `connie remove`, `connie doctor`, verbosity flag/env precedence, and end-to-end rejection of dangerous config (socket/kernel mounts, control-char env keys, bad resource values), including the legacy-`.connie/` and uninitialised diagnostics. |
| Docker | `tests/docker/` | End-to-end against real images and containers: `build-base`, `build`, `clean`, `run` lifecycle, runtime hardening (cap_drop, no-new-privileges, no SUID binaries), cgroup-v2 resource-limit enforcement, `.connie/` → XDG auto-migration trigger. Gated on `docker` being on `$PATH`. |
